...
def calc_ctmrg_env_rev(
    ...
    if logger.isEnabledFor(logging.INFO):
        if not converged:
            logger.warning("Custom VJP: ❌ did not converge , took %.2f seconds.(Steps: %d)", time.perf_counter() - t0, end_count)
        else:
            logger.info("Custom VJP: ✅ converged , took %.2f seconds.(Steps: %d)", time.perf_counter() - t0, end_count)
